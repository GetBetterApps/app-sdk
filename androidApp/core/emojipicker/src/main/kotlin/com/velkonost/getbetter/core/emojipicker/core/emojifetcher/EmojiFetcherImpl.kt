...
class EmojiFetcherImpl(
    ...
    override fun fetchEmojiData(callback: EmojiFetchCallback, url: String) {
        ...
        val okHttpClientBuilder = OkHttpClient().newBuilder().cache(cache = cache)
        ...
        okHttpClient.newCall(request = request).enqueue(
            responseCallback = object : Callback {
                ...
                override fun onResponse(call: Call, response: Response) {
                    callback.onFetchSuccess(data = response.body?.string().orEmpty())
                }
            },
        )
    }
