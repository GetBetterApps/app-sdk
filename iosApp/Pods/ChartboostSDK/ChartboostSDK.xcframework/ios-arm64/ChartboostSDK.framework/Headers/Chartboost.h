...
*/
+ (void)startWithAppID:(NSString *)appID appSignature:(NSString *)appSignature completion:(void (^)(
        CHBStartError *_Nullable error)
...
*/
+ (void)addDataUseConsent:(CHBDataUseConsent *)consent NS_SWIFT_NAME
...
*/
+ (void)clearDataUseConsentForPrivacyStandard:(CHBPrivacyStandard)privacyStandard NS_SWIFT_NAME

(clearDataUseConsent(for:));
...
To check the specific consent status for GDPR:
 ...
 ```
*/
+ (nullable __kindof CHBDataUseConsent

*)dataUseConsentForPrivacyStandard:(CHBPrivacyStandard)
